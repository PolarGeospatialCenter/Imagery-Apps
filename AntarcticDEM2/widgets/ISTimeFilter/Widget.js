...
define([
 ...
 connect.subscribe("refreshTime", lang.hitch(this, function (flag) {
                ...
                saveScene: function () {
                    ...
                    connect.publish("saveScene", [{sceneid: layer.mosaicRule.lockRasterIds[0], url: layer.url, date: this.orderedFeatures1[this.slider.value].attributes["acquisitiondate1"]}]);
                    registry.byId("savedSceneDetails").set("value", layer.mosaicRule.lockRasterIds[0] + "," + this.orderedFeatures1[this.slider.value].attributes["acquisitiondate1"]);
                    registry.byId("savedSceneID").set("value", this.orderedFeatures1[this.slider.value].attributes["Name"]);
                },
                ...
                refreshData: function () {
                    ...
                    this.dateField = "acquisitiondate1";
                },
...
this.noMinimizeDisplay = true;
                    ...
                    query.outFields = ["acquisitiondate1", "qual", "OBJECTID", "GroupName", "Category", "CenterX", "CenterY", "LowPS"];
                    query.where = "Category=1 AND acquisitiondate1 IS NOT NULL";
                    var queryTask = new QueryTask(this.config.urlElevationPGC);
                    query.orderByFields = ["acquisitiondate1"];
                    ...
                    queryTask.execute(query, lang.hitch(this, function (result) {
                        ...
                        for (var a = 0; a < result.features.length; a++) {
                            if (a === 0)
                            {
                                ...
                                if (locale.format(new Date(data[k].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"}) === locale.format(new Date(result.features[a].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"}) && data[k].attributes.LowPS !== result.features[a].attributes.LowPS) {
                        ...
                        for (var i = 0; i < this.lengthofsamples; i++) {
                            ...
                            distance.push({
                                ...
                                acqDate: data[i].attributes.acquisitiondate1,
                        ...
                        var request = esriRequest({
                            ...
                            content: {
                                ...
                                outFields: 'acquisitiondate1,Name,GroupName,Category,OBJECTID',
                        ...
                        request.then(lang.hitch(this, function (data) {
                            ...
                            for (var a in items) {
                                normalizedValues.push(
                                        {y: parseFloat(items[a].value),
                                            tooltip: (parseFloat(items[a].value)).toFixed(3) + ", " + locale.format(new Date(items[a].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"})});
                                normalizedValues1[a] = parseFloat(items[a].value);
                                itemInfo.push({
                                    acqDate: items[a].attributes.acquisitiondate1,
                            ...
                            this.chart.connectToPlot("default", lang.hitch(this, this.clickdata));
                                ...
                                html.set(this.pointgraph,"Elevation: <b>"+parseFloat((data.samples[0].value).split(" ")[0]).toFixed(1)+"m</b><br />Acquisition Date: <b>"+locale.format(new Date(data.samples[0].attributes.acquisitiondate1), {selector: "date", formatLength: "long"})+"</b>");
...
this.noMinimizeDisplay = true;
                    ...
                    var query = new Query();
                    query.geometry = point;
                    query.outFields = ["acquisitiondate1", "OBJECTID", "CenterX", "CenterY", "GroupName"];
                    ...
                    queryTask.execute(query, lang.hitch(this, function (result) {
                        ...
                        this.lengthofsamples = data.length;
                        for (var i = 0; i < this.lengthofsamples; i++) {
                            //   if((locale.format(new Date(data[i].attributes.acquisitiondate1), {selector: "date", datePattern: "yyyy/MM/dd"}) > this.config.OldestDate) &&(locale.format(new Date(data[i].attributes.acquisitiondate1), {selector: "date", datePattern: "yyyy/MM/dd"}) < this.config.LatestDate)){
                ...
                clickdata: function (evt)
                {
                    ...
                    if (type2 === "onclick")
                    {
                        ...
                        for (var g = 0; g < this.orderedFeatures1.length; g++)
                        {
                            ...
                            if (locale.format(new Date(this.orderedFeatures1[g].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"}) === locale.format(new Date(this.ElevationData[this.datesclick].acqDate), {selector: "date", datePattern: "dd/MM/yy"}) && this.orderedFeatures1[g].attributes.GroupName === this.ElevationData[this.datesclick].name)
                            {
                ...
                timeSliderShow: function () {
                    // var sliderNode = domConstruct.create(
                    // "div",
                    // {},
                    // this.timeSliderDiv,
                    // "first"
                    ...
                    query.outFields = ["acquisitiondate1", "GroupName", "LowPS", "Name"];
                    if (this.primaryLayer.url === this.config.urlElevationPGC) {
                        query.where =
                          "Category = 1 AND acquisitiondate1 IS NOT NULL";
                    ...
                    queryTask.execute(query, lang.hitch(this, function (result) {
                      ...
                      // for (var a = 0; a < result.features.length; a++) {
                      //     //   if((locale.format(new Date(result.features[a].attributes.acquisitiondate1), {selector: "date", datePattern: "yyyy/MM/dd"}) > this.config.OldestDate) && (locale.format(new Date(result.features[a].attributes.acquisitiondate1), {selector: "date", datePattern: "yyyy/MM/dd"}) < this.config.LatestDate)){
                      //     if (this.orderedFeatures1.length === 0)
                      //     {
                      //         this.orderedFeatures1.push(result.features[a]);
                      //         var k = 0;
                      //     } else {
                      //         if (locale.format(new Date(this.orderedFeatures1[k].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"}) === locale.format(new Date(result.features[a].attributes.acquisitiondate1), {selector: "date", datePattern: "dd/MM/yy"}) && this.orderedFeatures1[k].attributes.LowPS !== result.features[a].attributes.LowPS) {
                      ...
                      //  }
                      // }
                      console.log(this.orderedFeatures1, this.dateobj);

                      for (
                        var t = 0;
                        t <= this.orderedFeatures1.length - 1;
                        t++
                      ) {
                        this.dateobj.push({
                          date: locale.format(
                            new Date(
                              this.orderedFeatures1[
                                t
                              ].attributes.acquisitiondate1
                            ),
                            { selector: "date", datePattern: "dd/MM/yy" }
                          ),
                      ...
                      for (var a = 0; a < this.orderedFeatures1.length; a++) {
                        this.orderedDates.push(
                          this.orderedFeatures1[a].attributes[
                            "acquisitiondate1"
                          ]
                        );
                      }
                      ...
                      for (var i = 0; i < this.orderedDates.length; i++) {
                        labels[i] = locale.format(
                          new Date(this.orderedDates[i]),
                          { selector: "date", datePattern: "dd/MM/yy" }
                        ); //formatLength: "short"});
                      }
                        ...
                        this.sliderLabels = new HorizontalRuleLabels(
                          {
                            ...
                            labels: [
                              labels[0],
                              labels[this.orderedDates.length - 1],
                            ],
                          },
                          labelsNode
                        );
                    ...
                    if (this.slider === null) {
                      ...
                      if (this.appScene) {
                        for (var f in this.dateobj) {
                          if (this.appScene === this.dateobj[f].Scene) {
                            for (var j = 0; j < this.orderedDates.length; j++) {
                              if (
                                this.dateobj[f].date ===
                                locale.format(new Date(this.orderedDates[j]), {
                                  selector: "date",
                        ...
                        for (var r = 0; r < this.orderedFeatures1.length; r++) {
                          this.best.push(
                            this.orderedFeatures1[r].attributes.qual
                          );
                        }
                        ...
                        for (var z in this.orderedFeatures1) {
                          if (
                            this.orderedFeatures1[z].attributes.qual === index
                          ) {
                            var ind = z;
                            break;
                          }
                      ...
                      if (this.map.getLayer("primaryLayer")) {
                        ...
                        if (this.previousDateOnTimeSliderLandsat !== null) {
                          for (var i in this.orderedDates) {
                            if (
                              this.orderedDates[i] ===
                              this.previousDateOnTimeSliderLandsat
                            ) {
                              ind = i;
                            }
                          }
                      ...
                      html.set(
                        this.dateRange,
                        locale.format(new Date(this.orderedDates[ind]), {
                          selector: "date",
                          formatLength: "long",
                        }) +
                          "        Pixel Size: " +
                          Math.round(
                            this.orderedFeatures1[ind].attributes.LowPS
                          ) +
                          "m"
                      );
                      html.set(
                        this.temporalpro,
                        "Pick point on map to get temporal profile for that point"
                      );
                      //domStyle.set("loadingtime", "display", "none");
                    }), lang.hitch(this, function (error)
                    {
                ...
                sliderChange: function () {
                    ...
                    if (this.map.getLayer("primaryLayer")) {
                        ...
                        this.primaryLayer = this.map.getLayer("primaryLayer");
                        this.previousDateOnTimeSlider = this.orderedDates[this.slider.value];
                        registry.byId("arcticSceneID").set("value", this.orderedFeatures1[this.slider.value].attributes["Name"]);
                    } else if (this.map.getLayer("landsatLayer")) {
                        this.primaryLayer = this.map.getLayer("landsatLayer");
                        this.previousDateOnTimeSliderLandsat = this.orderedDates[this.slider.value];
                        registry.byId("landsatSceneID").set("value", this.orderedFeatures1[this.slider.value].attributes["Name"]);
                    }
                    this.sliderValue = this.slider.value;

                    if (this.sliderValue !== null) {
                        var aqDate = this.orderedFeatures1[this.slider.value].attributes["acquisitiondate1"];

                        this.object = [this.orderedFeatures1[this.slider.value].attributes.OBJECTID];
                        registry.byId("currentDate").set("value", aqDate);
                        html.set(this.dateRange, locale.format(new Date(aqDate), {selector: "date", formatLength: "long"}) + "        Pixel Size: " + Math.round(this.orderedFeatures1[this.slider.value].attributes.LowPS) + "m");
