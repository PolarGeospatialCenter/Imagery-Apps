            ...
            var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
                ...
                onOpen: function () {
                   ...
                   connect.publish("elevationOpen",({"flag": true}));
                    ...
                    if(layer && layer.url === this.config.urlElevationPGC)
             ...
             domStyle.set("loadingMeasure","display","none");
                ...
                refreshLayer: function(){
                    var layer = this.map.getLayer("primaryLayer");
                    if(layer.url !== this.config.urlElevationPGC)
                    {
                      html.set(this.errorHandlerElevation,"Elevation Profile not available for current Service.");
                      registry.byId("DialogElevation").show();
                       domStyle.set("DialogElevation","left","160px");
              ...
              this.chart = null;
                ...
                calculateAreaDistance: function(geometry){
                  ...
                  {    var geometrytype = "esriGeometryPolygon";
                      ...
                      var layersRequest1 = esri.request({
                        url: this.config.urlElevationPGC + "/Measure",
                        content: {
                 ...
                 var layersRequest = esri.request({
                    ...
                    layersRequest.then(lang.hitch(this, function (data) {
                        ...
                        for(var i=0;i<data.samples.length;i++){
                         ...
                         data.samples.sort(function(a,b){return a.locationId - b.locationId;});
               ...
               domStyle.set("loadingMeasure","display","none");
                ...
                /*getProfileOfRasters: function(geometry){
                    var getLayer = this.map.getLayer("primaryLayer");
                    var query  = new Query();
                    query.geometry = geometry;
                    query.orderByFields = ["acquisitiondate1"];
                    query.outFields = ["acquisitiondate1","OBJECTID"];
                    query.where = "Category = 1";
                    
                    var queryTask = new QueryTask(getLayer.url);
                    queryTask.execute(query, lang.hitch(this, function(result){
                        var array = [], acquisitionDate = [];
                        
                    for(var i in result.features){
                        array[i] = result.features[i].attributes.OBJECTID;
                        acquisitionDate[i] = locale.format(new Date(result.features[i].attributes.acquisitiondate1), {selector: "date", formatLength: "long"});
                    }
                    var distance = 0;
                ...
                var sampleDistance = distance/30;
                        var mosaicLock = new MosaicRule({"mosaicMethod": "esriMosaicLockRaster", "ascending": true, "mosaicOperation": "MT_FIRST", "lockRasterIds": array});
                    ...
                    for(var i=0;i<data.samples.length;i++){
                        ...
                        for(var i =0; i< length;i++){
                            if(item[i].rasterId !== array[j]){
                                arrayLength.push({
                                    length: i,
                                    index: j,
                                    objid: array[j]
                                });;
                                pixelValues[j] = item.splice(0,i);
                                pixelValues[j].sort(function(a,b){return a.locationId - b.locationId;});
                                i=0;length = item.length;j++;
                            }
                            else{
                                if(i === length - 1)
                                {     pixelValues[j] = item;
                        ...
                        for(var y=0;y<len;y++){
                            var obj = array[y];
                            for(var f=0;f<arrayLength.length;f++){
                            if(arrayLength[f].objid === obj)
                            {
                                present = "yes";
                                break;
                            }else{
                        ...
                        if(present === null)
                        {array.splice(y,1);
                            acquisitionDate.splice(y,1);
                            y--;len = array.length;
                        }}
                        arrayLength.sort(function(a,b){return b.length - a.length;});
                       
                       var tempDist = 0;
                        var arrayXAxis = [], arrayYAxis = [], minpixelvalue = 9999, maxpixelvalue = -9999;
                        this.location1 = [];
                        for (var i = 0; i < arrayLength[0].length; i++) {
                            if (i === 0) {
                                tempDist = 0;
                            } else
                                tempDist = tempDist + Math.pow(((Math.pow((pixelValues[arrayLength[0].index][i].location.x - pixelValues[arrayLength[0].index][i - 1].location.x), 2) + Math.pow(pixelValues[arrayLength[0].index][i].location.y - pixelValues[arrayLength[0].index][i - 1].location.y, 2))), 0.5);
                            arrayXAxis.push({
                                value: i + 1,
                                text: (parseInt(tempDist)).toString()
                            });
                            this.location1.push({
                                value: i + 1,
                                loc: pixelValues[arrayLength[0].index][i].location,
                                locationId: pixelValues[arrayLength[0].index][i].locationId
                            });}
                        var values =[],pixelValueRaster=[];
                        for(var h=0;h<array.length;h++){
                            
                            for(var s=0;s<pixelValues[h].length;s++){
                                for(var x=0;x<this.location1.length;x++){
                                    
                                if(pixelValues[h][s].locationId === (this.location1[x].locationId)){
                                 values = pixelValues[h][s].value.split(' ');
                            minpixelvalue = parseFloat(values[0]) < minpixelvalue ? parseFloat(values[0]) : minpixelvalue;
                            maxpixelvalue = parseFloat(values[0]) > maxpixelvalue ? parseFloat(values[0]) : maxpixelvalue;
                            arrayYAxis.push({
                                x: this.location1[x].value,
                                y: parseFloat(values[0]),
                                tooltip: (parseFloat(values[0])).toFixed(2)
                            });
                            break;
                        }}     }
                            pixelValueRaster[h] = arrayYAxis.splice(0,s);
                            //arrayYAxis.splice(0,s);
                        }
                        var pvrLength = pixelValueRaster.length;
                        for(var r=0;r<pvrLength;r++){
                            if(pixelValueRaster[r].length < 2){
                                pixelValueRaster.splice(r,1);
                                acquisitionDate.splice(r,1);
                                r--; pvrLength--;
                            }
                        }
                     ...
                     this.chart = new Chart("chartNodeElevation");
                        ...
                        this.chart.addAxis("x", {labels: arrayXAxis, labelSizeChange: true, title: "Distance", titleOrientation: "away",minorTicks: false, majorTickStep: 1});
                        for(var p=0;p<pixelValueRaster.length;p++){
                            {
                                if(p=== pixelValueRaster.length -1)
                                    this.chart.addSeries(acquisitionDate[p], pixelValueRaster[p]);
                                else
                                this.chart.addSeries(acquisitionDate[p], pixelValueRaster[p],{hidden: true});
                        ...
                        this.chart.render();
                        if(this.legend === null)
                         this.legend = new SelectableLegend({chart: this.chart, horizontal: true, outline: false}, "legendElevation");
                    else
