...
define([
    ...
    "esri/layers/ImageServiceParameters", "esri/dijit/PopupTemplate",
    "dijit/Tooltip",
    "dojo/dom-construct",
    "dojo/dom", "dojo/html",
    "dojo/dom-style",
    ...
    "dijit/form/NumberTextBox"
        ...
        function (
                ...
                ImageServiceParameters, InfoTemplate,
                Tooltip,
                domConstruct,
                dom, html, domStyle, connect, MosaicRule, Extent, registry, PanelManager, WidgetManager,domAttr) {
            ...
            var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
                ...
                closeOtherDialogs: function () {
                    ...
                    if (domClass.contains(x[1], "jimu-state-selected"))
                        x[1].click();
                    else if (domClass.contains(x[2], "jimu-state-selected"))
                        x[2].click();
                ...
                postCreate: function () {
                    ...
                    this.own(
                            ...
                            on(this.slopeRadio, "click", lang.hitch(this, this.domSelection, "Slope Degrees Map", this.slopeRadio)),
                            ...
                            on(this.contourRadio, "click", lang.hitch(this, function () {
                                ...
                                if (domClass.contains(this.contourRadio, "selected")) {
                                    ...
                                    if (registry.byId("smoothContour").checked)
                                        this.addContourLayer("Smooth Contour 25");
                                    else
                    ...
                    this.map.on("extent-change", lang.hitch(this, function (value) {
                        ...
                        if (this.map.getLayer("labelLayer")) {
                            if (value.lod.level < 9) {
                                ...
                                if (!this.map.getLayer("labelLayer").visible && domClass.contains(this.vectorRadio,"selected"))
                ...
                createZFactorSlider: function () {
                    var node = domConstruct.toDom('<div id="zfactorDiv">Shading Factor:<span style="font-weight:bold; color:black;margin-left:3px;" id="zfactorValue">8.0</span><button data-dojo-type="dijit/form/Button"  id="zfactorBtn" type="button">Apply</button><br />' +
                            '<div id="zfactorSlider" style="width:180px;display: inline-block;" data-dojo-type="dijit/form/HorizontalSlider" data-dojo-props="value:8.0,minimum: 0.0,maximum:10.0,discreteValues:11,intermediateChanges:false,showButtons:true">' +
                    ...
                    parser.parse(node).then(lang.hitch(this, function () {
                        ...
                        registry.byId("zfactorBtn").on("click", lang.hitch(this, function (value) {
                            ...
                            if (this.selectedRadio.id === "Hillshade Gray") {
                                ...
                                args.ZFactor = factor;
                            } else if (this.selectedRadio.id === "Slope Degrees Map")
                                args.ZFactor_2012823_20450_272 = factor;
                            rasterFunction.functionArguments = args;
                ...
                onOpen: function () {
                    ...
                    if (parameters[1])
                    {
                        ...
                        if (typeof (history.pushState) != "undefined") {
                            ...
                            var obj = {Page: "AntarcticDEM Explorer", Url: changeUrl};
                ...
                shareClicked: function (socialMedium) {
                    ...
                    if (socialMedium === "facebook")
                        ...
                        var share = "https://www.arcgis.com/home/socialnetwork.html?n=tw&t=AntarcticDEM Explorer" + "&u=" + shareUrl;
                    else
                ...
                elevationServices: function (renderer) {
                    ...
                    if (renderer !== "GreyHillshadeStretched") {
                        ...
                        if (renderer === "Hillshade Gray") {
                            ...
                            args.ZFactor = factor;
                            rasterFunction.functionArguments = args;
                        } else if (renderer === "Slope Degrees Map") {
                            domStyle.set("zfactorDiv", "display", "block");
                            var args = {};
                            var factor = registry.byId("zfactorSlider") ? parseFloat(registry.byId("zfactorSlider").get("value")) : 1.0;
                            factor = Math.pow(1.5, factor) - 0.5;
                ...
                populateLandsatService: function (renderer, radio) {
                    ...
                    if (domClass.contains(radio, "selected")) {
                        ...
                        if (registry.byId("buildDialog").open)
                            registry.byId("buildDialog").hide();
                             if(wm.getWidgetById("widgets/ISTimeFilter/Widget_15")) {
                            wm.getWidgetById("widgets/ISTimeFilter/Widget_15").landsatTurnOff(radio);
                    }
                    } else {
                        ...
                        if (renderer === "Pansharpened Enhanced with DRA") {
                           ...
                           // connect.publish("refreshTime", [{flag: true}]);
                            ...
                            if(wm.getWidgetById("widgets/ISTimeFilter/Widget_15")) {
                            wm.getWidgetById("widgets/ISTimeFilter/Widget_15").landsatTurnOn(radio);
                    }
                    }
                ...
                addVectorLayers: function () {
                    ...
                    mapServiceLayer.setImageFormat("png32");
                    ...
                    var featureLayer = new FeatureLayer(this.config.campLayer, {
                        id: "campLayer",
                        infoTemplate: new InfoTemplate(this.config.popupInfo),
                        outFields: ["*"],
                        visible: true
                    });
                    this.map.addLayer(featureLayer);
                    var featureLayer1 = new FeatureLayer(this.config.campLayer1, {
                        id: "campLayer1",
                        infoTemplate: new InfoTemplate(this.config.popupInfo1),
                        outFields: ["*"],
                        visible: true
                    });
                    this.map.addLayer(featureLayer1);
                    var featureLayer2 = new FeatureLayer(this.config.campLayer2, {
                        id: "campLayer2",
                        infoTemplate: new InfoTemplate(this.config.popupInfo2),
                        outFields: ["*"],
                        visible: true
                    });
                    this.map.addLayer(featureLayer2);
                    var featureLayer3 = new FeatureLayer(this.config.campLayer3, {
                        id: "campLayer3",
                        infoTemplate: new InfoTemplate(this.config.popupInfo3),
                        outFields: ["*"],
                        visible: true
                    });
                    this.map.addLayer(featureLayer3);
                    var featureLayer4 = new FeatureLayer(this.config.featureCollection.layers[0], {
                        id: "campLayer4",
                        infoTemplate: new InfoTemplate(this.config.featureCollection.layers[0].popupInfo),
                        outFields: ["*"],
                        visible: true
                    });
                    this.map.addLayer(featureLayer4);
                    console.log(featureLayer4);
                },
                turnOnOffVectorLayers: function () {
                    if (domClass.contains(this.vectorRadio, "selected")) {
                        ...
                        if (this.map.getLayer("campLayer"))
                            this.map.getLayer("campLayer").hide();
                        if (this.map.getLayer("campLayer1"))
                            this.map.getLayer("campLayer1").hide();
                        if (this.map.getLayer("campLayer2"))
                            this.map.getLayer("campLayer2").hide();
                        if (this.map.getLayer("campLayer3"))
                            this.map.getLayer("campLayer3").hide();
                        if (this.map.getLayer("campLayer4"))
                            this.map.getLayer("campLayer4").hide();
                    } else {
                        domClass.add(this.vectorRadio, "selected");
                        if (this.map.getLayer("campLayer"))
                            this.map.getLayer("campLayer").show();
                        if (this.map.getLayer("campLayer1"))
                            this.map.getLayer("campLayer1").show();
                        if (this.map.getLayer("campLayer2"))
                            this.map.getLayer("campLayer2").show();
                        if (this.map.getLayer("campLayer3"))
                            this.map.getLayer("campLayer3").show();
                        if (this.map.getLayer("campLayer4"))
                            this.map.getLayer("campLayer4").show();
                        if (this.map.getLayer("graticuleLayer"))
